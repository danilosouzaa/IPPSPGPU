...
int main( int argc, char **argv )
{
   ...
   // Inst_print(inst); //getchar();
    ...
    double timecreatemipc = omp_get_wtime();
    MIPCompact *mipC = MipC_create(inst, par, res,  -1, sumtpd, sumtms, _time);
    timecreatemipc =  (double) (omp_get_wtime() - timecreatemipc);

    _time = ( (double) tmp - (omp_get_wtime()-startT) );
    //printf("mipC->tpdSum %d  timecreatemipc %f, _time %f\n",MipC_getSumTPD(mipC), timecreatemipc, _time);
    //MipC_cutting_plane(mipC,sol,res,_time);

    if(Par_getCGraph(par))
   ...
   //MipC_solve( mipC, _time);
    ...
    Res_writeResults(par, MipC_mip(mipC), MipC_getSol(mipC), res, MipC_getBestObj(mipC), fp, argv, argc,  startT, sumtpd,sumtms, timecreatemipc);
