...
void Res_setTimestrengthening(Results *res, double value)
{
    assert(res);
    res->timestrengthening = value;
}

double Res_getTimestrengthening(Results *res)
{
    assert(res);
    return res->timestrengthening;
}
...
void Res_writeResults(Parameters *par, LinearProgram *mip, const Solution* sol, Results *res, double bo, FILE *fp, char **argv, int argc, double startT, int sumtpd, int sumtms, double timecreatemipc)
{
...
/*            minCut = Res_getNMinElementsCuts(res,r,LPC_JS);
    ...
    if(VERBOSE) {
        ...
        fprintf( fp, "%f ; %f ; %f ; %f ; %f ; %f ; %d ; %g ; %g ; %g ; %g ;\n", timesep,  timeopt, timeremadd, timecgraph, timecreatemipc, Res_getTimestrengthening(res), Res_getRound(res), bo, bc, roundedbo, bcrounded);
    } else
