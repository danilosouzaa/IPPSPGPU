...
LinearProgram * lp_create_cgsep(const double *xf, int nr, int nelem, int *idxelements, VecStr *nameelements, VecInt **elemrow, VecDbl **coefelemrows, VecDbl *rrhs, VecStr *rname, double timeleft)
{
    ...
    for(int i = 0 ; i < nr ; i ++) {
      ...
      //  printf("idx2[%d] %d, coef2[%d], %f\n", ni, idx2[ni], ni, coef2[ni]);
        ni++;
    }
    sprintf(name, "aRHS");
   ...
   // printf("idx2[%d] %d, coef2[%d], %f\n", ni, idx2[ni], ni, coef2[ni]);
    ...
    lp_add_row( mipCGSep, ni, idx2, coef2, namer, 'E', 0.0 );

    int idx3[ nelem+1];
    double coef3[ nelem+1];
    int ne3=0;
    for(int n = 0 ; n < nelem ; n++) {
        int e = idxelements[n];
        //int e = VInt_get(idxelements,n);
        ...
        idx3[ ne3] = aj3;
        coef3[ ne3] = xf[e];
     //   printf("idx3[%d] %d, coef3[%d], %f\n", ne3, idx3[ne3], ne3, coef3[ne3]);
        ne3++;
    }
    sprintf(name, "aRHS");
    ...
    idx3[ ne3] = a03;
    coef3[ ne3] = -1;
  //  printf("idx3[%d] %d, coef3[%d], %f", ne3, idx3[ne3], ne3, coef3[ne3]);
    fflush(stdout);
    ne3++;
   // lp_write_lp(mipCGSep, "teste.lp");getchar();
...
//    CutP_create_job_set(inst,lp, maxT);
    ...
    for ( int r=0 ; (r<lp_rows(lp)) ; ++r ) {
        ...
        // printf("nameR %s \n", nameR);
        // getchar();
        if(strncmp(nameR,"resR(",5)== 0 ){// || strncmp(nameR,"cutE",4)== 0){
            ...
            if(aux>0) {
                ...
                if(strncmp(nameR,"resR(",5)== 0) VInt_pushBack(rtype,RES_RR);
...
//    CutP_create_job_set(inst,lp, maxT);
    ...
    for ( int r=0 ; (r<lp_rows(lp)) ; ++r ) {
        ...
        // printf("nameR %s \n", nameR);
        // getchar();
        if(strncmp(nameR,"resR(",5)== 0 || strncmp(nameR,"cutRR",5)== 0){// || strncmp(nameR,"cutE",4)== 0){
            parseName( nameR, prefix, id );
            if(id[1] < mininterval || id[1] > maxinterval ) continue;
            //printf("mininstant %d maxinstant %d %s(%d,%d)\n", mininterval, maxinterval,nameR, id[0], id[1]);
            int *idx;
        ...
        if (fabs(xf[i])>EPS ) {
            ...
            if (tolower(name[0])=='x') {
                ...
                if (prefix[0]=='x') {
                    ...
                    maxHorizon = MAX(maxHorizon,t+Mode_duration(mode));
                    //      printf( "x(%d,%d,%d) xf: %g xfor %g, rdc: %f \n", j, m, t, xf[i], xf[origCols[i]], rdc[i]);
                }
            }
        }
...
//    CutP_create_job_set(inst,lp, maxT);
    ...
    for(int l = 0; l < ncols && contElem > 0; l++) {
        ALLOCATE_VECTOR_INI(varInConfs[l], int, ncols);
...
/*identify the set of important rows to compose the CG procedure with all constraints (RR -- convert z to x, cutRR, NR, Job Selection,) and conflicts about precedence relationship and project delay*/
CutCG *CutCG_create_and_identify_rows_all_conflitcts_cgraph_convertZtoX(  LinearProgram *lp, const CGraph *cgraph, const int *origCols, LinearProgram *origLP, const Instance *inst, double timeLeft, int mininstant, int maxinstant, int jump, int nround, int *horizon)
{
...
//    CutP_create_job_set(inst,lp, maxT);
    ...
    for ( int r=0 ; (r<lp_rows(lp)) ; ++r ) {
        ...
        if(strncmp(nameR,"resR",4)== 0 || strncmp(nameR,"cutRR",5)== 0){// || strncmp(nameR,"cutE",4)== 0){
            parseName( nameR, prefix, id );
            if(id[1] < mininterval || id[1] > maxinterval ) continue;
//            printf("\nmininstant %d maxinstant %d, %s, (%d,%d)\n", mininterval, maxinterval,nameR, id[0], id[1]);
            int *idx;
            ALLOCATE_VECTOR_INI(idx,int,lp_cols(lp));
            double *coef;
            ALLOCATE_VECTOR_INI(coef,double,lp_cols(lp));
            ...
            for(int i  = 0 ; i < size ; i++) {
                if (fabs(xf[idx[i]])>EPS ) {
                char nameE[256];
                lp_col_name(lp,idx[i],nameE);

                parseName( nameE, prefix, id );
                const Job*job = Inst_job(inst,id[0]);
                const Mode*mode = Job_mode(job,id[1]);
                int dur = id[2]-Mode_duration(mode)+1;
                if(dur<Job_est(job)) dur = Job_est(job);
                for( int dd = dur;dd<=id[2];dd++){
                    int ind = xIdx[id[0]][id[1]][dd];
                    if(ind != -1) {
                        if (fabs(xf[ind])>EPS ) {
                            char nameX[256];
                            lp_col_name(lp,ind,nameX);
                            //printf(" %s: ", nameE);
                            //printf(" %s, ind %d xf %f \n", nameX, ind ,xf[ind]);
                            if(elements[ind]==0 ) {
                                elements[ind] = 1;
                                idxelements[contElem] = ind;
                                VStr_pushBack(nameelements, nameX);
                                contElem++;
                            }
                            //VInt_pushBack(ridx[cont],idx[i]);
                            //VDbl_pushBack(rcoef[cont],coef[i]);
...
//                VChar_pushBack(rsense,sense);
  ...
  //  printf("cont %d, contElem %d, ncols %d\n\n", cont, contElem, ncols); //getchar();
...
void CutCG_add_cuts_model_CG_parallel( CutCG *ccg, const CGraph *cgraph, int cutType, LinearProgram *lp, const Instance *inst, double timeLeft,   double maxcuts, int nround, int horizon)
{
    ...
    //CREATE MIP CHAVATAL
      //printf("CREATE MIP CHAVATAL  ccg->cont %d\n" , ccg->cont); fflush(stdout);
    _time = ( (double) timeLeft - (omp_get_wtime()-startT) );
    //    printf("time %f", _time);fflush(stdout);
...
//      printf("Starting optimize \n");
    ...
    //CREAT CUT
    nSol = lp_num_saved_sols(mipCGSep);
   // printf("\nnSol %d \n", nSol);
...
void CutCG_add_cuts_CG_parallel( CutCG *ccg, const CGraph *cgraph, int cutType,LinearProgram *lp, const Instance *inst, double timeLeft, int continuous,  double maxcuts, int nround, int horizon)
{
    ...
    for( int r = 0 ; r < VDbl_size(ccg->cutrhs) ; r++) {
        ...
        */
        ...
        double newrhs = CutP_model_lift_cgraph( inst, cgraph, VStr_get(ccg->cutname,r),VInt_get(ccg->cutnelem,r),VInt_getPtr(ccg->cutElem[r]), VDbl_getPtr(ccg->cutCoef[r]), VDbl_get(ccg->cutrhs,r), lp,_time, horizon);
