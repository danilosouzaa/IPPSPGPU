...
The default setting (0) makes an automatic choice, with a slight preference for speed.
Settings 1-3 increasingly shift the focus towards being more careful in numerical computations.
With higher values, the code will spend more time checking the numerical accuracy of intermediate results,
and it will employ more expensive techniques in order to avoid potential numerical issues.*/
...
void lp_config_grb_params(LinearProgram *lp)
{
    ...
    if ( lp->parallel != INT_NOT_SET){
        if(lp->parallel)
            GRBsetintparam( env, GRB_INT_PAR_THREADS, lp->parallel );
        else
...
DataGRBCallback *DataGRB_create( const Instance *inst,  double timeLeft, int lifting, double maxcuts){
    ...
    DataGRBCallback *dt = ( DataGRBCallback *) malloc( sizeof(DataGRBCallback) );
