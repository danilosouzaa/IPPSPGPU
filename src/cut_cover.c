...
/*Lifted MIP to separate cover cuts,  also verify dominance between generated cuts in function by z*/
void CutC_add_cuts_cover_model_byz_parallel( CutC *ccr, LinearProgram *lp, const int *origCols,  LinearProgram *origLP, const Instance *inst, double timeLeft, int continuous, int lifting,  int nround )
{
...
#define MAX_IDX 16
    ...
    // allocation of z
    double ***z;
    ALLOCATE_VECTOR( z, double **, nJobs );
    ALLOCATE_VECTOR( z[0], double *, nJobs*nModes );
    ALLOCATE_VECTOR( z[0][0], double, nJobs*nModes*nTimes );
    for ( int j=1 ; (j<nJobs) ; ++j )
        z[j] = z[j-1] + nModes;

    for ( int j=0; (j<nJobs) ; ++j )
        for ( int m=0 ; (m<nModes) ; ++m )
            z[j][m] = z[0][0] + j*nModes*nTimes + m*nTimes;
    ...
    // for all variables
    // filling z from fractional solution
    for ( i=0 ; (i<lp_cols(lp)) ; ++i )
    {
        if(maxRC != -1.0)
            if(rdc[i] > maxRC) continue;
        ...
        if (tolower(name[0]=='z'))
        {
            ...
            //  if (fabs(xf[i])>1e-5) {
            if (prefix[0]=='z')
                z[idx[0]][idx[1]][idx[2]] = zf[i];
            // }
        }
    }

    /* z indexes pre-processed problem*/
    int ***zIdx;
    ALLOCATE_VECTOR( zIdx, int **, nJobs );
    ALLOCATE_VECTOR( zIdx[0], int *, nJobs*nModes );
    ALLOCATE_VECTOR( zIdx[0][0], int, nJobs*nModes*nTimes );
    for ( int j=1 ; (j<nJobs) ; ++j )
        zIdx[j] = zIdx[j-1] + nModes;

    for ( int j=0; (j<nJobs) ; ++j )
        for ( int m=0 ; (m<nModes) ; ++m )
            zIdx[j][m] = zIdx[0][0] + j*nModes*nTimes + m*nTimes;
    // filling default values
    {
        int *it, *itEnd;
        it = zIdx[0][0];
        ...
        for ( int j= 0 ; j< nJobs ; ++j )
        {
            ...
            for ( int m=0 ; (m<nModes) ; ++m )
            {
                ...
                for ( int ir=0 ; (ir<Mode_nResR(mode)) ; ++ir )
                {
                    ...
                    if(zIdx[j][m][t] == -1 || fabs(z[j][m][t]) == 1.0) continue;

                    sumX[t][idzR][j][m] += z[j][m][t];
                    sumRC[t][idzR][j][m] += rdc[zIdx[j][m][t]];
    ...
    for(int r = 0 ; r < Inst_nResR(inst); r++ )
    {
        ALLOCATE_VECTOR(ccr->cutElem[r],VecInt*,nTimes*Inst_nProjects(inst));
        ALLOCATE_VECTOR(ccr->cutCoef[r],VecDbl*,nTimes*Inst_nProjects(inst));
        for(int nc = 0 ; nc < nTimes*Inst_nProjects(inst); nc++ )
        {
            ...
            if(nj && nwt && nzt )
            {
                ...
                for(int iy = 0 ; iy <nj ; iy++)
                {
                    ...
                    for(int jmw = 0 ; jmw < Job_nModes(jobw) ; jmw++)
                    {
                        ...
                        coefwr[ijm] = vecTSameJWcoef[t][idj[iy]][r][jmw];
                        ijm++;
                        ...
                        for(int jmw2 = 0 ; jmw2 < Job_nModes(jobw); jmw2++)
                        {
                            ...
                            {
                                idxmoderes[imr] = wIdx[t][r][idj[iy]][jmw2];
                                coefmoderes[imr] = -1.0;
                                imr++;
                            }
                        }
                            ...
                            if (tolower(nameC[0]=='z'))
                            {
                                ...
                                if (zf2[i]>EPS)
                                {
                                    if (prefix[0]=='z')
                                    {
                                        ...
                                        if(zIdx[j][m][t] == -1) continue;

                                        cutIdxCo[c].a =  zIdx[j][m][t];
                                        cutIdxCo[c].b = 1.0;
                                        slack += zf[cutIdxCo[c].a]*cutIdxCo[c].b;
    ...
    CutC_add_cuts_cover_model_byz_new_const_parallel( ccr, lp, origCols,  origLP, inst,  timeLeft,  continuous,  lifting,   nround,  nTimes, zIdx,z,nCutR);
...
void CutC_add_cuts_cover_model_byz_new_const_parallel( CutC *ccr, LinearProgram *lp, const int *origCols,  LinearProgram *origLP, const Instance *inst, double timeLeft, int continuous, int lifting,  int nround, int nTimes,  int ***zIdx, double ***z, int*nCutR)
{
...
#define MAX_IDX 16
    ...
    ALLOCATE_VECTOR( sumX, double ***, nTimes );
    ALLOCATE_VECTOR( sumRC, double ***, nTimes );
    ALLOCATE_VECTOR( rhs, double **, nTimes );
    for ( int t=0; (t<nTimes) ; ++t )
    {
        ALLOCATE_VECTOR( sumX[t], double ***, Inst_nResR(inst) );
        ALLOCATE_VECTOR( sumRC[t], double ***, Inst_nResR(inst) );
        ALLOCATE_VECTOR( rhs[t], double *,  Inst_nResR(inst) );
        for ( int r=0 ; (r<Inst_nResR(inst)) ; ++r )
        {
            ALLOCATE_VECTOR_INI( sumX[t][r], double **, Inst_nProjects(inst));
            ALLOCATE_VECTOR_INI( sumRC[t][r], double **, Inst_nProjects(inst));
            ALLOCATE_VECTOR( rhs[t][r], double,  Inst_nProjects(inst) );
            for ( int p=0 ; (p<Inst_nProjects(inst)) ; ++p )
            {
                ALLOCATE_VECTOR_INI( sumX[t][r][p], double *, Inst_nJobs(inst) );
                ALLOCATE_VECTOR_INI( sumRC[t][r][p], double *, Inst_nJobs(inst));
                for ( int j=0 ; (j<Inst_nJobs(inst)) ; ++j )
                {
                    ALLOCATE_VECTOR_INI( sumX[t][r][p][j], double, Inst_nMaxModes(inst) );
                    ALLOCATE_VECTOR_INI( sumRC[t][r][p][j], double, Inst_nMaxModes(inst));
                }
            }
        }
    }
    ...
    for(int t  = 0 ; t < nTimes ; t++)
    {
        ...
        for ( int r=0 ; (r<Inst_nResR(inst)) ; ++r )
        {
            ALLOCATE_VECTOR(namesbin[t][r], VecStr*,  Inst_nProjects(inst));
            ALLOCATE_VECTOR( addedTRJM[t][r], int **, Inst_nProjects(inst) );

            ALLOCATE_VECTOR( zIdxm[t][r], int **, Inst_nProjects(inst) );
            ALLOCATE_VECTOR( wIdx[t][r], int **, Inst_nProjects(inst) );

            for ( int p=0 ; (p<Inst_nProjects(inst)) ; ++p )
            {
                namesbin[t][r][p] = VStr_create( STR_SIZE );
                ALLOCATE_VECTOR( addedTRJM[t][r][p], int *, nJobs );
                ALLOCATE_VECTOR( zIdxm[t][r][p], int *, nJobs );
                ALLOCATE_VECTOR( wIdx[t][r][p], int *, nJobs );
                for(int j = 0 ; j < nJobs ; j++)
                {
                    ALLOCATE_VECTOR( addedTRJM[t][r][p][j], int, nModes );
                    ALLOCATE_VECTOR( zIdxm[t][r][p][j], int, nModes );
                    ALLOCATE_VECTOR( wIdx[t][r][p][j], int, nModes );
                    for(int m = 0 ; m <nModes ; m++)
                    {
                        addedTRJM[t][r][p][j][m] = 0;
                        zIdxm[t][r][p][j][m] = -1;
                        wIdx[t][r][p][j][m] = -1;
                        sumX[t][r][p][j][m] = 0;
                        sumRC[t][r][p][j][m] = 0;
    ...
    for(int t  = 0 ; t < nTimes ; t++)
    {
        ...
        for ( int r=0 ; (r<Inst_nResR(inst)) ; ++r )
        {
            ALLOCATE_VECTOR( idxJ[t][r], VecInt*, Inst_nProjects(inst) );
            ALLOCATE_VECTOR( yIdx[t][r], int *, Inst_nProjects(inst) );
            for ( int p=0 ; (p<Inst_nProjects(inst)) ; ++p )
            {
                idxJ[t][r][p] = VInt_create();
                ALLOCATE_VECTOR( yIdx[t][r][p], int, nJobs );
                for(int j = 0 ; j < nJobs ; j++)
                    yIdx[t][r][p][j] = -1;
            }
        }
    ...
    ALLOCATE_VECTOR( idxResRZ, VecInt***, Inst_nResR(inst) );
   // ALLOCATE_VECTOR_INI( idxResRZ[0], VecInt**, Inst_nResR(inst)*(nTimes));
   // ALLOCATE_VECTOR_INI( idxResRZ[0][0], VecInt*, Inst_nResR(inst)*(nTimes)*Inst_nProjects(inst) );
    ALLOCATE_VECTOR( idxResRW, VecInt***, Inst_nResR(inst) );
    //ALLOCATE_VECTOR_INI( idxResRW[0], VecInt**, Inst_nResR(inst)*(nTimes) );
   // ALLOCATE_VECTOR_INI( idxResRW[0][0], VecInt*, Inst_nResR(inst)*(nTimes)*Inst_nProjects(inst) );

   // for ( int i=1 ; (i<Inst_nResR(inst)) ; ++i )
   // {
     //   idxResRZ[i] = idxResRZ[i-1] + nTimes +Inst_nProjects(inst);
     //   idxResRW[i] = idxResRW[i-1] + nTimes +Inst_nProjects(inst);
   // }
    for ( int r=0 ; (r<Inst_nResR(inst)) ; ++r ){
        ALLOCATE_VECTOR( idxResRZ[r], VecInt**,nTimes );
        ALLOCATE_VECTOR( idxResRW[r], VecInt**,nTimes );
        for ( int t=0 ; (t<nTimes) ; ++t )
        {
            ALLOCATE_VECTOR( idxResRZ[r][t], VecInt*,Inst_nProjects(inst) );
            ALLOCATE_VECTOR( idxResRW[r][t], VecInt*, Inst_nProjects(inst) );
            for ( int p=0 ; p<Inst_nProjects(inst) ; ++p )
            {
                idxResRZ[r][t][p] = VInt_create();
                idxResRW[r][t][p] = VInt_create();
            }
        }
    }
    ...
    for ( int i=0 ; (i<nTimes) ; ++i )
    {
        ...
        for ( int j=0 ; (j<nJobs) ; ++j )
        {
            ...
            for ( int r=0 ; (r<Inst_nResR(inst)) ; ++r )
            {
                ALLOCATE_VECTOR( vecTSameJWcoef[i][j][r], double*, Inst_nProjects(inst));
                for ( int p=0 ; p<Inst_nProjects(inst) ; ++p )
                {
                    ALLOCATE_VECTOR( vecTSameJWcoef[i][j][r][p], double, Inst_nMaxModes(inst));
                    for ( int m=0 ; (m<Inst_nMaxModes(inst)) ; ++m )
                    {
                        vecTSameJWcoef[i][j][r][p][m] = 0;
                    }
                }
            }
        }
    ...
    for(int o = 0 ; o < nTimes; o++)
    {
        ...
        for ( int r=0 ; (r<Inst_nResR(inst)) ; ++r )
        {
            ALLOCATE_VECTOR_INI(objbin[o][r],double*,Inst_nProjects(inst));
            ALLOCATE_VECTOR_INI(nColB[o][r],int,Inst_nProjects(inst));
            for ( int p=0 ; p<Inst_nProjects(inst) ; ++p )
            {
                ALLOCATE_VECTOR_INI(objbin[o][r][p],double,nJobs*Inst_nMaxModes(inst)*nTimes);
            }
        }
    }


    for (int ro = 0 ; ro < lp_rows(lp) ; ro++ )
    {
        lp_row_name(lp,ro,name);
        if(strncmp(name,"resRZ",5)== 0)
        {
            ...
            t= id[1];
            p = id[2];
            rhs[t][r][p] = lp_rhs(lp,ro);
            int idx[lp_cols(lp)];
            double coef[lp_cols(lp)];
            int size = lp_row(lp,ro,idx,coef);
            ...
            for(int c = 0 ; c < size ; c++)
            {
                ...
                if (prefix[0]=='z')
                {
                    ...
                    const int m = idx[1];
                    sumX[t][r][p][j][m] += zf[i];
                    sumRC[t][r][p][j][m] += rdc[i];
                    if(addedTRJM[t][r][p][j][m] == 0)
                    {
                        char vnameZ[STR_SIZE];
                        sprintf( vnameZ, "z(%d,%d)", j, m);
                        zIdxm[t][r][p][j][m] =  nColB[t][r][p] ;
                        nColB[t][r][p]++;
                        VStr_pushBack( namesbin[t][r][p], vnameZ );
                        if(yIdx[t][r][p][j]==-1)
                        {
                            char vnameY[STR_SIZE];
                            sprintf( vnameY, "y(%d)", j);
                            objbin[t][r][p][nColB[t][r][p]] = 0.0;
                            yIdx[t][r][p][j] = nColB[t][r][p];
                            VStr_pushBack( namesbin[t][r][p], vnameY );
                            VInt_pushBack( idxJ[t][r][p], j );
                            nColB[t][r][p]++;
                        }

                        char vnameW[STR_SIZE];
                        sprintf( vnameW, "w(%d,%d)", j, m);
                        wIdx[t][r][p][j][m] =  nColB[t][r][p];
                        nColB[t][r][p]++;
                        VStr_pushBack( namesbin[t][r][p], vnameW );

                        objbin[t][r][p][zIdxm[t][r][p][j][m]] = - 0.1;
                        objbin[t][r][p][wIdx[t][r][p][j][m]] = 0.0;

                        vecTSameJWcoef[t][j][r][p][m] = coef[c];
                        VInt_pushBack( idxResRZ[r][t][p], zIdxm[t][r][p][j][m] );
                        VInt_pushBack( idxResRW[r][t][p], wIdx[t][r][p][j][m] );

                        addedTRJM[t][r][p][j][m] = 1;
                    }
                }
            }
        }
    ...
    for ( int t=0 ; t<nTimes; ++t )
    {
        for ( int r=0 ; (r<Inst_nResR(inst)) ; ++r )
        {
            for ( int p=0 ; (p<Inst_nProjects(inst)) ; ++p )
            {
                ...
                if(nColB[t][r][p]==0) continue;
                mipCutm = lp_create();
                double _time = (double) timeLeft - (omp_get_wtime()-init);
                lp_set_print_messages(mipCutm,0);
                lp_set_max_seconds(mipCutm,_time);
                lp_set_nThreads(mipCutm,1);
                lp_add_bin_cols( mipCutm, nColB[t][r][p], objbin[t][r][p], VStr_ptr(namesbin[t][r][p]) );
                ...
                const VecInt *vidzm = idxResRZ[r][t][p];
                const VecInt *vidwm = idxResRW[r][t][p];

                const int nwt = VInt_size(vidwm);
                const int nzt = VInt_size(vidzm);

                const VecInt *vj = idxJ[t][r][p];
                const int nj = VInt_size( vj );
                int *idj =  VInt_getPtr( (VecInt *) vj );
                ...
                if(nj && nwt && nzt )
                {
                    ...
                    for(int iy = 0 ; iy <nj ; iy++)
                    {
                        if( yIdx[t][r][p][idj[iy]]==-1)continue;
                        ...
                        for(int jmw = 0 ; jmw < Job_nModes(jobw) ; jmw++)
                        {
                            ...
                            double coefmoderes[Job_nModes(jobw)+1];
                            int imr = 0;
                            if( wIdx[t][r][p][idj[iy]][jmw] == -1 || zIdxm[t][r][p][idj[iy]][jmw] ==-1 ) continue;
                            idSameJW[iii] = wIdx[t][r][p][idj[iy]][jmw];
                            iii++;
                            idwr[ijm] = wIdx[t][r][p][idj[iy]][jmw];
                            coefwr[ijm] = vecTSameJWcoef[t][idj[iy]][r][p][jmw];
                            ijm++;


                            if(sumX[t][r][p][idj[iy]][jmw]>EPS)
                            {
                                idzyv[ijz] = zIdxm[t][r][p][idj[iy]][jmw];
                                coefzyv[ijz] = sumX[t][r][p][idj[iy]][jmw];
                                ijz++;
                            }
                            ...
                            for(int jmw2 = 0 ; jmw2 < Job_nModes(jobw); jmw2++)
                            {
                                if( wIdx[t][r][p][idj[iy]][jmw2] == -1 ) continue;
                                if(vecTSameJWcoef[t][idj[iy]][r][p][jmw2]<=vecTSameJWcoef[t][idj[iy]][r][p][jmw])
                                {
                                    idxmoderes[imr] = wIdx[t][r][p][idj[iy]][jmw2];
                                    coefmoderes[imr] = -1.0;
                                    imr++;
                                }
                            }
                            idxmoderes[imr] = zIdxm[t][r][p][idj[iy]][jmw];
                            coefmoderes[imr] = 1.0;
                            imr++;


                            char namewz[STR_SIZE];
                            sprintf( namewz, "cons(3).(%d,%d,%d)#(%d,%d)",t,r,p,idj[iy],jmw);
                            // printf("cons(3).(%d,%d)#(%d,%d)",t,r,idj[iy],jmw);
                            lp_add_row( mipCutm, imr, idxmoderes, coefmoderes, namewz, 'L', 0.0);
                        ...
                        int coly = yIdx[t][r][p][idj[iy]];
                        idSameJW[iii] = coly;
                        coefSameJW[iii] = 1.0;
                        char namewy[STR_SIZE];
                        iii++;
                        sprintf( namewy, "cons(1).(%d,%d,%d)(%d)",t,r,p,idj[iy]);
                        lp_add_row( mipCutm, iii, idSameJW, coefSameJW, namewy, 'E',0.0);
                    ...
                    char name[STR_SIZE];
                    sprintf( name, "cons(2).(%d,%d,%d)",t,r,p);
                    lp_add_row( mipCutm, ijm, idwr, coefwr, name, 'E', rhs[t][r][p]);

                    char nameel[STR_SIZE];
                    sprintf( nameel, "cons(4).(%d,%d,%d)",t,r,p);
                    int idel[1];
                    double coefel[1];
                    idel[0] = lp_col_index(mipCutm,"e");
                    coefel[0] = 1.0;
                    ...
                    char namezyv[STR_SIZE];
                    sprintf( namezyv, "cons(5).(%d,%d,%d)",t,r,p);
                    // printf("ijz %d, size(idzyv) %d size(coefzyv) %d\n", ijz, sizeof(idzyv)/sizeof(idzyv[0]), sizeof(coefzyv)/sizeof(coefzyv[0])); fflush(stdout);
                    lp_add_row( mipCutm, ijz, idzyv, coefzyv, namezyv, 'E', -1 );
                    ...
                    char namev[STR_SIZE];
                    sprintf( namev, "cons(6).(%d,%d,%d)",t,r,p);
                    int idev[1];
                    double coefv[1];
                    idev[0] = lp_col_index(mipCutm,"v");
                    coefv[0] = 1;
                    ...
                    if(st==0)
                    {
                        ...
                        if(valueZ >= 0.0005)
                        {
                            ...
                            if(slack-(ny-1)>=0.0005){
                                ...
                                char nameCut[STR_SIZE];
                                //printf( "cutRR(%d,%d)#%d\n", r, t, nCut );
                                sprintf( nameCut, "cutRRZ(%d,%d,%d)#%d#%d", r, t, p, nround, nCut );
                                VStr_pushBack(ccr->cutname[r],nameCut);
                                // double coe[c];
    //                        getchar();
                                CutP_quick_sort(cutIdxCo, c);
    ...
    for(int r  = 0 ; r < Inst_nResR(inst) ; r++)
    {
        for(int t  = 0 ; t < nTimes ; t++)
        {
            for(int p  = 0 ; p < Inst_nProjects(inst) ; p++)
            {
                VInt_free( &idxResRZ[r][t][p] );
                VInt_free( &idxResRW[r][t][p] );
    ...
    for(int t  = 0 ; t < nTimes ; t++)
    {
        for(int r  = 0 ; r < Inst_nResR(inst) ; r++)
        {
            ...
            for(int p  = 0 ; p < Inst_nProjects(inst) ; p++)
            {
                ...
                VStr_free(&namesbin[t][r][p]);
                free(objbin[t][r][p]);
                VInt_free(&idxJ[t][r][p]);
                for(int j = 0 ; j < nJobs ; j++)
                {
                    free(zIdxm[t][r][p][j]);
                    free(wIdx[t][r][p][j]);
                    free(addedTRJM[t][r][p][j]);
                    free(sumX[t][r][p][j]);
                    free(sumRC[t][r][p][j]);
                }
                free(sumRC[t][r][p]);
