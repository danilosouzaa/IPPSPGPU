...
Instance *Inst_create_rcpsp( const char *dir, const char *instance )
{
    ...
    ALLOCATE_VECTOR_INI( inst->idxIniResRProj, int, inst->nProjects );
...
#define N_FIELDS 5
    ...
    while ( (s=fgets( line, LINE_SIZE, f)) ) {
        ...
        if (strstr( s, "mode duration" )) {
            ...
            inst->idxIniResRProj[p->index] = idxStartRLocalR;
...
int *Inst_getCapNR( const Instance *inst )
{
    assert( inst!=NULL );
...
int *Inst_getCapRR( const Instance *inst )
{
    assert( inst!=NULL );
...
int Inst_idxResRProj( const Instance *inst, int idxProj )
{
    assert( inst!=NULL );
    assert( idxProj >= 0);
    assert( idxProj < inst->nProjects);

    return inst->idxIniResRProj[idxProj];
}
