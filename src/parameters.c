...
void Par_checkArgs(Parameters *par, char **argv, int argc)
{
    ...
    for(int n = 1; n< argc ; n++) {
        ...
        if (strcmp(argv[n],"-variablesz") == 0) {
            n++;
            par->variablesz =  atoi(argv[n]);
            continue;
        }
        if (strcmp(argv[n],"-cgraph") == 0) {
            n++;
            par->cgraph =  atoi(argv[n]);
            continue;
        }
    }
}

int Par_getCGraph( const Parameters *par ){
    return par->cgraph;
}
int Par_getVariablesZ( const Parameters *par ){
    return par->variablesz;
}
