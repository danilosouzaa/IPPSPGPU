...
//    printf("cut %d \n",  cut); getchar();
 //   printf("cut %d ALLOCATION %d\n",  cut, VInt_size(cutsSense)); getchar();
...
int CutP_separationParallel(CutPool *cutP, FILE *fp2, const Parameters *par,  Results *res, int cr, int cp, int cc, int co, int cg, int cggpur2, int cgcpu,  int** maxTJM, int* maxTJ, double startT, double timeLeft)
{
    ...
    CutCG *ccggpur2;
    CutCG *ccg;
    int thr = cr+cp+cc+co+cg+cgcpu+cggpur2;
    double cutTimeCR = 0.0,  cutTimeCP = 0.0,   cutTimeCC = 0.0, cutTimeCO = 0.0, cutTimeCG = 0.0;
               ...
               // printf("cr ID %d\n", omp_get_thread_num()); fflush(stdout);//getchar();
                ...
                if(par->lifting){
                    if(Par_getVariablesZ(par)==1)
                        CutC_add_cuts_cover_model_byz_parallel( ccr, cutP->mip, origCols, cutP->mip, par->inst,_time, par->continuous, par->lifting, Res_getRound(res));
                    else
        ...
        #pragma omp section
        {
            if(cg)
            {
                ...
                int horizon=0;
                 ...
                 if(Par_getVariablesZ(par)==1){
//                    printf("initializing ccg"); fflush(stdout);
                    ccg = CutCG_create_and_identify_rows_all_conflitcts_cgraph_convertZtoX(  cutP->mip, cgraph, origCols, cutP->mip, par->inst, _time,par->mininstant, par->maxinstant, par->jump,  Res_getRound(res), &horizon);
  //                  printf("ccg initialized"); fflush(stdout);
...
//    int cutjs = 0;
  ...
  /*  if(cjs)
    ...
    fflush(stdout);
    //lp_write_lp(cutP->mip,"verificandobyz.lp");
   // printf("verificando byz");
    //getchar();
...
void CutP_quick_sort_vec_int ( int *idx, int *idx2, int n)
{
    if (n < 2)
        return;
    int i = 0, j = 0, p = 0;
    ...
    CutP_quick_sort_vec_int(idx, idx2, i);
    CutP_quick_sort_vec_int(idx+i, idx2+i, n - i);
}
...
double CutP_model_lift_cgraph( const Instance *inst, const CGraph *cgraph, const char* cutname, int cutnelem, int *cutElem, double *cutCoef, double rhs, LinearProgram *lp, double timeLeft, int horizon){
  ...
  //  printf("%s:", cutname);fflush(stdout);
//    int time = id[1];
    ...
    for(int e = 0 ; e < cutnelem ;e++){
     ...
     //   printf(" %f * %s + ", objbin[e], name);
       ...
       for(int res = 0 ; res < Mode_nResR(mode); res++){
            for(int ttd = t ; ttd< t+Mode_duration(mode) ; ttd++){
